...
export async function getEphemeralAgent({config, serviceAgent} = {}) {
  ...
  if(record.expires < now) {
    // clear expired record from cache and retry; note that there's a chance
    // the cache was updated with a new promise and we will now delete it so
    // it must be recomputed, but this will only occur when the zcaps for
    // a service agent are not refreshed more than a few minutes before their
    // expiry, which would be bad practice
    EPHEMERAL_AGENT_CACHE.delete(key);
    return getEphemeralAgent({config, serviceAgent});
  }

  // return `_next` for testing purposes only
  const {capabilityAgent, zcaps, next: _next} = record;
  return {capabilityAgent, zcaps, _next};
}
...
export function _resetEphemeralAgentCache({
  ttl = EPHEMERAL_AGENT_MAX_TTL
} = {}) {
  // set new TTL and reset cache
  EPHEMERAL_AGENT_MAX_TTL = ttl;
  _createEphemeralAgentCache();
}
...
function _createEphemeralAgentCache() {
  ...
  EPHEMERAL_AGENT_CACHE = new LruCache({
    ...
    // efficiency
    maxSize: 1000,
    // TTL for ephemeral agents; use half of the max to rely on rotations
    // and because this gets refreshed on cache hits
    maxAge: Math.floor(EPHEMERAL_AGENT_MAX_TTL / 2),
...
async function _getUncachedEphemeralAgent({config, serviceAgent, key} = {}) {
  ...
  longer than the expiration date on the zcaps they use. Note that the docs
  on `getEphemeralAgent()` say these agents last 5 minutes -- here we
  internally double that to allow for some buffer but we could adjust down
  later if we need to without a breaking change.

  A timer will be set to run a rotation operation after half of that time has
  passed, provided that the agent will not expire by then.
  ...
  // compute rotation time as half-way through max TTL; if agent will expire
  // prior to then, do not schedule a rotation as the zcaps will not last
  ...
  record.next = new Promise(resolve => setTimeout(() => {
    // only start rotation if record is still present in the cache
    if(!EPHEMERAL_AGENT_CACHE.cache.has(key)) {
      return resolve(null);
    }
    ...
    promise
      .then(() => EPHEMERAL_AGENT_CACHE.cache.set(key, promise))
      .catch(() => {});
    resolve(promise);
  }, halfTtl));
