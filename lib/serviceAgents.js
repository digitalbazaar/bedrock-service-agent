...
bedrock.events.on('bedrock.init', async () => {
  ...
  const cacheConfig = coerceCacheConfig(cfg.caches.serviceAgent);
  SERVICE_AGENT_CACHE = new LruCache(cacheConfig);
...
export async function getEphemeralAgent({config, serviceAgent} = {}) {
  ...
  const promise = EPHEMERAL_AGENT_CACHE.cache.peek(key);
  const record = await promise;

  // clear expired record from cache (if it hasn't already changed) and retry
  const now = new Date();
  if(record.expires < now) {
    if(EPHEMERAL_AGENT_CACHE.cache.peek(key) === promise) {
...
function _createEphemeralAgentCache() {
  ...
  EPHEMERAL_AGENT_CACHE = new LruCache({
    ...
    // to `ttl` (this does not impact its `expires` tracking, once expired
...
async function _getUncachedEphemeralAgent({config, serviceAgent, key} = {}) {
  ...
  record.next = new Promise(resolve => setTimeout(() => {
    ...
    const current = EPHEMERAL_AGENT_CACHE.cache.peek(key);
    if(!current) {
      return resolve(null);
    }
    current.then(currentRecord => {
      ...
      if(!(currentRecord === record &&
        current === EPHEMERAL_AGENT_CACHE.cache.peek(key))) {
        return resolve(null);
      }
      ...
      promise.then(() => {
        if(current === EPHEMERAL_AGENT_CACHE.cache.peek(key)) {
