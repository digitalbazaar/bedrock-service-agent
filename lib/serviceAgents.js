...
export async function getEphemeralAgent({config, serviceAgent} = {}) {
  ...
  // FIXME: need to change this to schedule the creation of an ephemeral agent
  // independently so it can be returned if ttl < FIVE_MINUTES; if one isn't
  // available, one must be created and the caller will need to wait
  // FIXME: schedule code to create a new uncached ephemeral agent after
  // maxAge - 1 minutes; check for its existence and update cache and return
  // it
