...
let EPHEMERAL_AGENT_MAX_CACHE_TTL = FIVE_MINUTES;
...
export function _resetEphemeralAgentCache({
  ttl = EPHEMERAL_AGENT_MAX_CACHE_TTL
} = {}) {
  // set new TTL and reset cache
  EPHEMERAL_AGENT_MAX_CACHE_TTL = ttl;
  _createEphemeralAgentCache();
}
...
function _createEphemeralAgentCache() {
  ...
  EPHEMERAL_AGENT_CACHE = new LruCache({
    ...
    maxAge: EPHEMERAL_AGENT_MAX_CACHE_TTL,
...
async function _getUncachedEphemeralAgent({config, serviceAgent, key} = {}) {
  ...
  // compute rotation time max TTL; if agent will expire prior to then, do not
  // schedule a rotation as the zcaps will not last
  const rotationTime = Date.now() + EPHEMERAL_AGENT_MAX_CACHE_TTL;
