...
// FIXME: add method to get ephemeral signer to optimize
/**
 * Gets an ephmeral agent that has been delegated all of the zcaps in
...
export async function getEphemeralAgent({config, serviceAgent} = {}) {
  ...
  if(ttl < FIVE_MINUTES) {
    // kick off creating another uncached ephemeral agent and store it in
    // the LRU cache
    // FIXME: could set this on the record instead ...
    // and when the record expires, add it back to the cache?
    // record.next = _getUncachedEphemeralAgent({config, serviceAgent});
    //EPHEMERAL_AGENT_CACHE.cache.set(key, )
  }
