...
let EPHEMERAL_AGENT_MAX_TTL = TEN_MINUTES;
...
bedrock.events.on('bedrock.init', async () => {
  ...
  if(cfg._EPHEMERAL_AGENT_MAX_AGE) {
    EPHEMERAL_AGENT_MAX_TTL = cfg._EPHEMERAL_AGENT_MAX_AGE;
  }
  ...
  EPHEMERAL_AGENT_CACHE = new LruCache({
    ...
    // efficiency
    maxSize: 1000,
    // TTL for ephemeral agents
    maxAge: EPHEMERAL_AGENT_MAX_TTL,
...
async function _getUncachedEphemeralAgent({config, serviceAgent, key} = {}) {
  ...
  prevents rotation. Rotation involves making a network request to sign a
  delegated zcap, so the service must be able to receive that response before
  going idle for rotation to work properly.

  Ephemeral agents should be short-lived to increase the security posture, but
  not so lived that there is no advantage to using them. An attempt is made to
  make ephemeral agents last `EPHEMERAL_AGENT_MAX_TTL`, but they cannot last
  longer than the expiration date on the zcaps they use. A timer will be set to
  run a rotation operation after half of that time has passed, provided that
  the agent will not expire by then.
  ...
  they are not used, the cache will expire agents
  ...
  // the max expiration date for any delegated zcap; add one minute to
  // `EPHEMERAL_AGENT_MAX_TTL` to ensure its zcaps will be usable by the agent
  // for at least a minute, which is longer than any request using the zcaps
  // should reasonably take
  const maxExpires = new Date(
    Date.now() + EPHEMERAL_AGENT_MAX_TTL + ONE_MINUTE);
  ...
  // calculate the earliest expiration date for any of the zcaps
  ...
  // expire the agent a minute before any zcap might expire; see above comments
  const expires = new Date(earliestExpires.getTime() - ONE_MINUTE);
  const record = {capabilityAgent, zcaps, next: null, rotating: false, expires};

  // if `expires` is before the rotation time, do not schedule a rotation as
  // the zcaps will not last
  const halfTtl = Math.floor(EPHEMERAL_AGENT_MAX_TTL / 2);
  const rotationTime = Date.now() + halfTtl;
  if(expires.getTime() < rotationTime) {
    return record;
  }

  // schedule potential cache record rotation five minutes from now
  setTimeout(() => {
    ...
    // FIXME: add once test is written
    //.catch(e => e);
  }, halfTtl);
