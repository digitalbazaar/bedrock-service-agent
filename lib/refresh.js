...
export async function refreshZcaps({serviceType, config, signal} = {}) {
  ...
  try {
    ...
    if(policy?.refresh === false) {
      logger.error(
        ...
        `"${config.id}" disabled with "refresh=false"; zcap refresh ` +
        'disabled for config; fix and update config to re-enable.');
      return {
        refresh: {enabled: false, after: 0}
      };
    }
    ...
    if(status === 404 || status === 403 ||
      ...
      logger.error(
        `Zcap refresh policy for service type "${serviceType}" and config ` +
        `"${config.id}" not found or not allowed; zcap refresh prevented ` +
        `for config; fix to re-enable, retrying again later.`, {error});
    } else {
      logger.error(
        'Could not fetch zcap refresh policy during config record refresh ' +
        `for "${serviceType}"; trying refresh again later.`, {error});
    }
    // error fetching refresh policy, try again later
    const now = Date.now();
    return {
      refresh: {
        enabled: true, after: now + DEFAULT_POLICY_ERROR_REFRESH_AFTER
      },
      error
    };
  }
  ...
  const now = Date.now();
  let after = now + DEFAULT_MAX_TTL_BEFORE_REFRESH;
  ...
  const results = await Promise.all([...zcaps].map(
    ([key, capability]) => queue.add(async () => {
      ...
      let result;
      if(now < refreshTime) {
        // do not refresh, too early; do not return though, to allow for
        // `refreshTime` to modify `after`
        result = {capability, refreshed: false, referenceId: key};
      } else {
        // refresh
        result = await _refreshCapability({
...
function _getRefreshTime({policy, capability}) {
  ...
  // set refresh time to the earliest possible time based on expiry time minus
  // `maxTtlBeforeRefresh`; good policies will give more than a 10 minute
  // `maxTtlBeforeRefresh` to allow for clock skew plus checks before expiry;
  // no need to add/guess clock skew here
  const expiryTime = Date.parse(capability.expires);
  return expiryTime - maxTtlBeforeRefresh;
}
