...
export async function get({config, serviceType} = {}) {
  ...
  if(record.next && !record.rotating) {
    ...
    Promise.resolve(record.next)
      .then(() => DOCUMENT_STORE_CACHE.set(key, record.next))
...
async function _getUncachedDocumentStore({config, serviceType, key}) {
  ...
  // FIXME: get ephemeral signer instead to optimize; it is known that
  // the ephemeral signer will last at least 5 minutes
  ...
  // return doc store and flags for rotation to help prevent some cold cache
  // issues
